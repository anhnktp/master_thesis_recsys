...
def plot_emb(df):
	...
	plt.xticks(df.n_emb.unique())
	plt.subplot(2,2,3)
	fig = sns.lineplot(x='n_emb', y='epoch_loss', hue='model', style='model',
		markers=True, markersize=10, linewidth=2, data=df)
	fig.set(ylabel="BCELogLoss")
	fig.set(xlabel="Number of Embeddings")
	plt.xticks(df.n_emb.unique())
...
def plot_loss(df):
	...
	plt.subplot(2,2,1)
	plt.subplots_adjust(hspace=0.4)
	fig = sns.lineplot(x='epoch_loss', y='best_hr', hue='model', style='model',
		markers=True, markersize=10, linewidth=2, data=df.round(4))
	fig.set(ylabel="HR@10")
	fig.set(xlabel="BCELogLoss")
	plt.subplot(2,2,2)
	fig = sns.lineplot(x='epoch_loss', y='best_ndcg', hue='model', style='model',
		markers=True, markersize=10, linewidth=2, data=df.round(4))
	fig.set(ylabel="NDCG@10")
	fig.set(xlabel="BCELogLoss")