...
class SECGMF(nn.Module):

    def __init__(self, num_user, num_item, num_factor, dropout, model_type, GMF_model=None, MLP_model=None):
        super(SECGMF, self).__init__()
        """
        ...
        self.MLP_lin = nn.Linear(in_features=num_factor*(num_factor - 1), out_features=num_factor)
        self.predict_layer = nn.Linear(in_features=2*num_factor, out_features=1)
    ...
    def _init_weight_(self):
        ...
        if 'pre' not in self.model:
            ...
            nn.init.kaiming_uniform_(self.MLP_lin.weight, a=1, nonlinearity='sigmoid')
            ...
            self.embed_user_MLP.weight.data.copy_(self.MLP_model.embed_user_GMF.weight)
            self.embed_item_MLP.weight.data.copy_(self.MLP_model.embed_item_GMF.weight)
    ...
    def forward(self, user, item):
        ...
        stack = torch.stack([embed_user_MLP, embed_item_MLP], dim=-2)
        stack = torch.unsqueeze(stack, dim=1)
        output_SEC = F.relu(self.conv1(stack))
        output_SEC = output_SEC.view(output_SEC.size()[0], -1)
        output_SEC = self.MLP_lin(output_SEC)
        concat = torch.cat((output_GMF, output_SEC), -1)
